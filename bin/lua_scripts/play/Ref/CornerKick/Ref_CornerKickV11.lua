...
gPlayTable.CreatePlay{
  ...
  ["rush"] = {
    ...
    Leader   = USE_CHIP and task.goCmuRush(LEADER_POS1, _, _, flag.allow_dss) or task.goCmuRush(LEADER_POS1, _, 400),
  ...
  ["pass"] = {
    ...
    Assister = USE_CHIP and task.chipPass(PASS_POS, 280) or task.goAndTurnKick(PASS_POS, 450),
    Leader   = task.goCmuRush(LEADER_POS1, _, _, flag.allow_dss),--task.InterTouch(SHOOT_POS),
  ...
  ["fix"] = {
    ...
    Assister = task.stop(),
    Leader   = task.goCmuRush(LEADER_POS1, _, _, flag.allow_dss),
    Special  = HALF and task.stop() or task.leftBack(),
    Middle   = HALF and task.stop() or task.rightBack(),
  ...
  ["shoot"] = {
    ...
    Assister = task.stop(),
    Leader   = task.InterTouch(),--USE_CHIP and task.InterTouch(SHOOT_POS) or task.waitTouchNew(SHOOT_POS),--task.touch(),
    Special  = HALF and task.stop() or task.leftBack(),
    Middle   = HALF and task.stop() or task.rightBack(),
    Defender = task.defendMiddle(),
    Goalie   = task.goalie(),
