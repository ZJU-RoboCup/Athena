-- by GTY 2016.3.30
local Gate_pos = CGeoPoint:new_local(600,0)
local MIDDLE_POS1 = ball.refSyntYPos(CGeoPoint:new_local(370,110))
local REC_POS = ball.refSyntYPos(CGeoPoint:new_local(-250,220))
local KICK_POS1 = ball.refAntiYPos(CGeoPoint:new_local(-200,250))
local KICK_POS2 = ball.refAntiYPos(CGeoPoint:new_local(360,270))
local PASS_POS = ball.refAntiYPos(CGeoPoint:new_local(362,270))
...
["passBall"] = {
	...
	Assister = task.goAndTurnKick(REC_POS,300),
...
["touchpass"] = {
	switch = function()
		if  bufcnt(player.kickBall("Special"),1,150) then
			print("gotoball")
			return "gotoball"
		end
	end,
	...
	Leader   = task.goCmuRush(KICK_POS2),
	Special  = task.receivePass(PASS_POS,650),
	Middle   = task.goCmuRush(MIDDLE_POS1),
	Defender = task.singleBack(),
	Goalie   = task.goalie(),
	match    = "{ALSMD}"
...
["gotoball"] = {
	switch = function()
		if  bufcnt(player.toTargetDist("Leader") <20,1,50) then
			print("kickball")
			return "kickball"
		end
	end,
...
["kickball"] = {
	...
	Leader   = task.waitTouchNew(),--task.receiveShoot(Gate_pos),
	Special  = task.leftBack(),
	Middle   = task.goCmuRush(MIDDLE_POS1),
	Defender = task.rightBack(),
	Goalie   = task.goalie(),
