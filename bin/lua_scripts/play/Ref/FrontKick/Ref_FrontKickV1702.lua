...
if useLearnRound then
  ...
  TURN_KICK    = task.passToPos(ball.goRush(),500)
end

local PassPos = CGeoPoint:new_local(450, 0)
...
["goThreeJam"] = {
  switch = function ()
    if bufcnt(player.toTargetDist("Defender") < 20 , "fast", 120) then
      ...
      if  pos.isChip("Defender",true) == false then
        return "goPass"
      else
        return "goPass"--"dribble"
      end
    end
  end,
  Assister = GO_BACK_BALL,
...
["dribble"] = {
  ...
  Assister = task.slowGetBall(PassPos),
  Leader   = task.continue(),
  Special  = task.continue(),
  Middle   = task.continue(),
  Defender = task.continue(),
...
["chipPass"] = {
  ...
  Assister = task.chipPass(PassPos,180),
  Leader   = task.continue(),
  Special  = task.continue(),
  Middle   = task.rightBack(),
  Defender = task.leftBack(),
