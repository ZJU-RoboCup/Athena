...
local RUN_POS_1    = {
  ...
  ball.refAntiYPos(CGeoPoint:new_local(270, 220)),
}
local RUN_POS_2    = {
  ball.refAntiYPos(CGeoPoint:new_local(210, -15)),
...
local PASS_POS = CGeoPoint:new_local(270, 220)--pos.passForTouch(CGeoPoint:new_local(380, 120))
...
["pass"] = {
  ...
  Assister = task.chipPass(PASS_POS, 210),
...
["fix"] = {
  ...
  Special  = task.leftBack(),
  Leader   = task.goCmuRush(RUN_POS_1[3]),
...
["kick"] = {
  ...
  Assister = task.goSupportPos("Special"),
  Special  = task.leftBack(),
  Leader   = task.InterTouch(RUN_POS_1[3]),--task.InterTouch(RUN_POS_1[3]),--
  Middle   = task.defendMiddle(),
  Defender = task.rightBack(),
  Goalie   = task.goalie(),
  match    = "{ALM}[SD]"
