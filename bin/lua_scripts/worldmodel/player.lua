...
function toTargetDir(p)
	pos=p
	return function(role)
		return (pos - player.pos(role)):dir()
	end
end
--need to modify
...
function canFlatPassToPos(role, targetpos)
	...
	for i = 1, param.maxPlayer do
		if player.valid(i) and i ~= gRoleNum["Leader"] and player.pos(i):dist(p2) > 30 then
			local dist = seg:projection(player.pos(i)):dist(player.pos(i))
			local isprjon = seg:IsPointOnLineOnSegment(seg:projection(player.pos(i)))
			if dist < 12 and isprjon then
				return false
			end
		end
