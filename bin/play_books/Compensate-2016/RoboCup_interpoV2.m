...
FIX_RATIO_GLOBAL=COMPENSATION_DATA(1); %总体修改
FIX_RATIO_15=COMPENSATION_DATA(2);        %具体角度修改
FIX_RATIO_25=COMPENSATION_DATA(3);
FIX_RATIO_35=COMPENSATION_DATA(4);
FIX_RATIO_45=COMPENSATION_DATA(5);
FIX_RATIO_55=COMPENSATION_DATA(6);
FIX_RATIO_65=COMPENSATION_DATA(7);
FIX_RATIO_75=COMPENSATION_DATA(8);
FIX_RATIO_85=COMPENSATION_DATA(9);

%-------------------------------------------------------------------------
%输入原始数据
matInput=importdata('CompensateRecordFile.txt');
...
i=1;
while true
    [curSize,~]=size(matInput);
    
    if i>curSize
        break;
    end
    
    speedOut=matInput(i,SPEED_OUT);
    if (speedOut<600 || speedOut>950)
       matInput(i,:)=[];
    else
        i=i+1;
    end
...
for i=1:1:m
    Xcur=matInput(i,SPEED_IN);
    Ycur=matInput(i,ANGLE_IN);
    Zcur=matInput(i,FIX_ANG);
    
    if(Ycur*Zcur<0)
        matInput(i,FIX_ANG)=abs(matInput(i,FIX_ANG));
    else
        matInput(i,FIX_ANG)=abs(matInput(i,FIX_ANG))*-1;
    end
    
    matInput(i,ANGLE_IN)=abs(matInput(i,ANGLE_IN));
end

Xi=matInput(:,SPEED_IN);
Yi=matInput(:,ANGLE_IN);
Zi=matInput(:,FIX_ANG);
...
mesh(Xo,Yo,Zo_final);
hold off;
axis([RANGE_SPEED_LO,RANGE_SPEED_HI,RANGE_ANGLE_LO,RANGE_ANGLE_HI,-10,30]);
...
strFileNameXls=[strName,'.xlsx'];
strFileNameTxt=[strName,'.txt'];
