...
int ConfigFile::ReadVector(DataMap *dmap, char *str, int _type)
{
  ...
  do {
    ...
    if (eptr != cptr) {
      while ((*eptr != 0) && !isdigit(*eptr) && (*eptr != '-') )
	eptr++;
    }
...
bool ConfigReader::ReadFile(char *_fname)
{
  ...
  sprintf(filepath, "%s\\%s.cfg", path, _fname);
...
{
  CR_SETUP(test, data1, CR_DOUBLE);
  CR_SETUP(test, data2, CR_INT);
  CR_SETUP(test, data3, CR_INT);
  CR_SETUP(test, data6, CR_STRING);

  configreader.Print(stderr);

  // lets do some data access
  fprintf(stderr, "Array 1 %i: ", VARSIZE(data1));
  for (uint i = 0; i < VARSIZE(data1); i++)
    fprintf(stderr, "%f, ", VDVAR(data1)[i]);
  fprintf(stderr, "\ndata 2 %d\n", IVAR(data2));
