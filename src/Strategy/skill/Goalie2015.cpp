...
//版本1.1 
//改动：
//		1.如果身后有看不到的球，向前微调。 check!
//		2.清球角度，若清球方向有人，换方向
//		3.还原2013版本中“Attack”状态 check!
...
CGoalie2015::CGoalie2015()
{
	state = NORMAL;

}

void CGoalie2015::plan(const CVisionModule* pVision)
{
...
CPlayerCommand* CGoalie2015::execute(const CVisionModule* pVision)
{
	if (subTask()){
...
bool CGoalie2015::needtoClearBall(const CVisionModule* pVision)
{
	bool result = false;
...
bool CGoalie2015::checkWeHaveHelper(const CVisionModule* pVision)
{
	bool result = false;
...
bool CGoalie2015::veryDangerous(const CVisionModule* pVision)
{
	const MobileVisionT& objBall = pVision->Ball();
	const CGeoPoint pBall = objBall.Pos();
	const CVector vBallVel = objBall.Vel();
	const double dBallVel = vBallVel.mod();
	CGeoLine lBallTrag = CGeoSegment(pBall, pBall + vBallVel * 300);
	CGeoLine lGateLine = CGeoSegment(pGateLetf, pGateRight);
	CGeoLineLineIntersection T2Ginter = CGeoLineLineIntersection(lBallTrag, lGateLine);

	if (pBall.dist(pGateCenter)<450 && dBallVel>50 && T2Ginter.Intersectant()
		&& T2Ginter.IntersectPoint().y() > -65 && T2Ginter.IntersectPoint().y() < 65
		)
		return true;
	else
		return false;
...
bool CGoalie2015::needtoAttackEnemy(const CVisionModule* pVision)
{
	const PlayerVisionT enemy = pVision->TheirPlayer(BestPlayer::Instance()->getTheirBestPlayer());
	if (!enemy.Valid()) return false;
	const PlayerVisionT& me = pVision->OurPlayer(task().executor);
	const MobileVisionT& ball = pVision->Ball();

	bool ball_moving = false, safe = false;

	CGeoPoint pBall = ball.Pos();
	CVector vBallVel = ball.Vel();
	double dBallVel = vBallVel.mod();
	double angBallVel = ball.Vel().dir();

	CGeoPoint pPlayer = me.Pos();
	CGeoPoint pEnemy = enemy.Pos();

	CGeoLine lBallTrag = CGeoSegment(pBall, pBall + vBallVel * 100);
	CGeoLine lGateLine = CGeoSegment(pGateLetf, pGateRight);
	CGeoLineLineIntersection T2Ginter = CGeoLineLineIntersection(lBallTrag, lGateLine);
	CGeoPoint pPredict = T2Ginter.IntersectPoint();

	if (dBallVel > 20) ball_moving = true;
	//safe?
	if (pBall.dist(pGateCenter) > 300)
	{
		if (dBallVel > pBall.dist(pGateCenter)*0.8 && T2Ginter.Intersectant() && T2Ginter.IntersectPoint().y()<120 && T2Ginter.IntersectPoint().y()>-120)
			safe = false;
		else
			safe = true;
	}
	else if (pBall.dist(pGateCenter) > 150)
	{
		if (ball_moving && !T2Ginter.Intersectant() || !ball_moving) safe = true;
		else if (ball_moving && T2Ginter.IntersectPoint().y()<110 && T2Ginter.IntersectPoint().y()>-110) safe = false;
		else safe = true;

		if (abs(Normalize((pEnemy - pGateCenter).dir() - (pBall - pGateCenter).dir())) > 45 * degree)
			safe = false;
	}
	else if (pBall.dist(pGateCenter) <= 150)
	{
	...
	if (safe && InOurPenaltyArea(pEnemy, -PENALTY_BUFFER))
		return true;
	else
		return false;
