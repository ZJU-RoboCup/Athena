...
#include <winsock2.h>// add by magic
//#include <sys/poll.h>//linux file
#include <Ws2tcpip.h>
#include <inaddr.h>
#include <stdio.h>
//#include <netdb.h>//linux file
//#include <unistd.h>//linux file
//#include <fcntl.h>//linux file
//#include <vector>
//#include <iostream>
#include "netraw.h"
//#include "common.h"
...
//uint32 
ULONG Address::getInAddr() const// add by magic
{
  const sockaddr_in *s = (sockaddr_in*)(&addr);
  //printf("s->sin_addr.s_addr:%ld\n",s->sin_addr.s_addr);//delete by sz
...
bool UDP::only_once = true;

bool UDP::open(int port, bool share_port_for_multicasting, bool multicast_include_localhost, bool blocking)
{
  // open the socket
	//if(fd >= 0) ::close(fd);//delete :: by magic
	if(fd >= 0) closesocket(fd);
  fd = socket(PF_INET, SOCK_DGRAM, 0);

  // set socket as non-blocking
  /*int flags = fcntl(fd, F_GETFL, 0);//delete by magic
 if(flags < 0) flags = 0;
  fcntl(fd, F_SETFL, flags | (blocking ? 0 : O_NONBLOCK));
*/
  u_long iMode = blocking? 1: 0;//by magic 
  //int flags = ioctlsocket(fd, F_GETFL, 0);//delete by magic
  //if(flags < 0) flags = 0;
  //ioctlsocket(fd, F_SETFL, flags | (blocking ? 0 : O_NONBLOCK));
  ...
  if (multicast_include_localhost) {
    ...
    if (setsockopt(fd, IPPROTO_IP, IP_MULTICAST_LOOP, (const char*)&yes, sizeof(yes))==SOCKET_ERROR) {
...
bool UDP::addMulticast(const Address &multiaddr,const Address &interfaces)
{
  static const bool debug = false;
  struct ip_mreq imreq;
  imreq.imr_multiaddr.s_addr = multiaddr.getInAddr();
  imreq.imr_interface.s_addr = interfaces.getInAddr();
  /*printf("multiaddr:0x%08X 0x%08X\n",
	  (unsigned)multiaddr.getInAddr(),
	  (unsigned)INADDR_ANY);
  printf("interfaces:0x%08X 0x%08X\n",
	  (unsigned)interfaces.getInAddr(),
	  (unsigned)INADDR_ANY);*/
  if(debug){
  ...
  int ret = setsockopt(fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, reinterpret_cast<char*>(&imreq), sizeof(imreq));
...
bool UDP::send(const char *data,int length,const Address &dest)
{
...
int UDP::recv(char *data,int length,Address &src)
{
...
bool UDP::wait(int timeout_ms) const
{
  ...
  return true;//(poll(&pfd,1,timeout_ms) == 1);
