...
private:
	CVector speedForTime_FM(double frame, const CVisionModule* pVision);   //计算若干帧以后的速度
	CVector speedForDist_FM(double dist, const CVisionModule* pVision); //计算若干距离后的速度
	double timeForDist_FM(double dist, const CVisionModule* pVision);      //计算球运动若干距离的时间
	CGeoPoint posForTime_FM(double frame, const CVisionModule* pVision);   //计算若干帧以后的绝对位置
private:
	CVector _ballVel;
